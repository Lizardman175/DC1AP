...
namespace DC1AP.Items
{
    ...
    internal class StarterWeapons
    {
        ...
        // TODO values not in the right order/places.  getting 5 wind, sky, mimic for crysknife.  Must be further offset in there.  The 8 is gone?

        // Hard coded weapons for now
        // CrysKnife
        private static Weapon toan = new(0x0106, 12, 25, 70, 8, 55, 4, 0, 0, 0, 0, 8, 0, 5, 0, 0, 0, 5, 5, 0, 0, 0);
        // Steel slingshot
        ...
        internal static void GenerateWeapons()
        {
            if (Options.StarterWeapons)
            {
                for (int i = 0; i < Math.Min(6, starterWeapons.Length); i++)
                {
                    Memory.WriteStruct(MiscAddrs.WeaponAddrs[i] + MiscAddrs.WeaponOffset, cheatWeapons[i]);
                }
            }
        }
    }
